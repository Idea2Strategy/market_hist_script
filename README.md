...
Wikipedia S&P 500 목록/변경 이력
    ...
    기존 파일 없음   기존 파일 있음
      3년치 수집      마지막 시점부터 갱신
          ...
          ┌─────┴─────┐
          │           │
 market_data/csv/  market_data/parquet/
       *.csv             *.parquet
                │
                ▼
       데이터 검사 및 보고서 생성
...
실행하면 먼저 저장 형식을 묻습니다. `1` 또는 Enter를 입력하면 CSV, `2`를 입력하면 Parquet으로 저장합니다.
...
1. 선택한 형식에 맞춰 `market_data/csv/` 또는 `market_data/parquet/` 폴더를 생성합니다.
...
CSV와 Parquet은 서로 독립적으로 관리됩니다. 예를 들어 CSV로 수집한 뒤 다음 실행에서 Parquet을 선택하면 CSV를 변환하지 않고 `market_data/parquet/`에 새로운 Parquet 데이터 수집을 시작합니다. 이전 버전에서 `market_data/` 바로 아래에 저장한 Parquet 파일도 자동 이동하거나 변환하지 않습니다.
...
현재 두 검사 스크립트는 Parquet 전용이며 `market_data/parquet/*_5min_historical.parquet`를 전부 읽습니다. CSV로 저장한 데이터는 이 검사 대상에 포함되지 않습니다. 데이터가 많으면 검사에도 메모리와 시간이 필요하며, `data_report.py`는 `report/` 폴더를 자동 생성하고 기존 `report/data_audit_report.txt`를 새 결과로 덮어씁니다.

## 저장 데이터 형식

파일명 형식은 선택한 저장 방식에 따라 다음과 같습니다.
